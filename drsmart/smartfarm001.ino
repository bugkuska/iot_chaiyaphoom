...
// Modbus Master
ModbusMaster node1;  // Slave ID1: SoilMoisture Sensor
ModbusMaster node2;  // Slave ID2: 3IN1 Humidity, Temperature and Light
...
void setup() {
  ...
  node1.begin(1, Serial2);  // Slave ID1: SoilMoisture Sensor
  node2.begin(2, Serial2);  // Slave ID2: 3IN1 Humidity, Temperature and Light
...
// Function Read sensor: SoilMoisture Slave ID:1
void readSensorssoil1() {
  uint8_t result1;
  float soil1 = node1.getResponseBuffer(2) / 10.0f;
  Serial.println("Get Soil data");
  result1 = node1.readHoldingRegisters(0x0000, 3);  // Read 2 registers starting at 1)
...
// Function to read 3IN1 sensor: Humidity, Temperature, and Light Slave ID:2
void read3in1sensor(float &humi2, float &temp2, int &light2) {
  uint8_t result2;
  humi2 = (node2.getResponseBuffer(0) / 10.0f);
  temp2 = (node2.getResponseBuffer(1) / 10.0f);
  light2 = (int)node2.getResponseBuffer(2);  // แปลง light2 เป็น int

  Serial.println("Get 3IN1 Data");
  result2 = node2.readInputRegisters(0x0000, 3);  // Read 2 registers starting at 1)
  if (result2 == node2.ku8MBSuccess) {
    ...
    Blynk.virtualWrite(V1, humi2);
    Blynk.virtualWrite(V2, temp2);
    Blynk.virtualWrite(V3, light2);
...
void update3in1sensor() {
  float humi2, temp2;
  int light2;
  read3in1sensor(humi2, temp2, light2);  // เรียกใช้ฟังก์ชันเพื่ออ่านค่าจากเซ็นเซอร์ 3IN1
}
...
void displaySensorDataOnLCD() {
  ...
  int light2 = (int)node2.getResponseBuffer(2);
